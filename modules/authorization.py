...
class Auth:
    ...
    def validate_user(self, user, password): #Flask user object
        ...
        if bcrypt.checkpw(password=password, hashed_password=hashed):
            return True
        return False
...
class Sessions:
    ...
    def getUserFromSession(self, session_id):
        return self.mongo.db.login.find_one(
            {"_sessionId":session_id}
        )
    ...
    def newSession(self, user_id):
        ...
        self.mongo.db.login.update_one(
            {"_id":user_id}, # Sessions expire after 2 hours
            {"$set":{"_sessionId": token, "expires":time.time()+(60*60*2)}}
        )
