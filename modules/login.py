...
class User():
    ...
    def __init__(self, _id, email, password_hash, salt, session_id):
        self.id = _id
        self.email = email
    ...
    #     return auth.validate_user(password, self.password_hash, self.salt)

class MongoUser(User): # Compose a User from a MongoDB document.
...
def load_user(email):
    ...
    if user != None:
        return User(user["_id"], user["email"], user["pw_hash"], user["salt"])
    else:
        return None